...
const LiveMap: React.FC<LiveMapProps> = ({ drivers, jobs, selectedJobId, route = [] }) => {
  ...
  useEffect(() => {
    ...
    // High Clarity Voyager Map (Better than pure black)
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-slate-100">
      ...
      <div className="absolute top-6 left-6 z-[1000] flex flex-col space-y-3">
        ...
        <button onClick={() => { setIsLocked(false); setLockedTargetId(null); }} className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-2xl border transition ${isLocked ? 'bg-blue-600 text-white border-blue-400' : 'bg-white text-gray-400 border-gray-200'}`}>
      ...
      {showSidebar && !selectedJobId && (
        <div className="absolute right-6 top-6 bottom-6 z-[1000] w-80 bg-white/95 backdrop-blur-md rounded-[2.5rem] border border-gray-200 shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right-12">
          <div className="p-6 border-b border-gray-100 flex space-x-2 bg-gray-50/50">
            <button onClick={() => setActiveTab('DRIVERS')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition ${activeTab === 'DRIVERS' ? 'bg-slate-900 text-white shadow-lg' : 'text-gray-400 hover:text-gray-900'}`}>Units</button>
            <button onClick={() => setActiveTab('MISSIONS')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition ${activeTab === 'MISSIONS' ? 'bg-slate-900 text-white shadow-lg' : 'text-gray-400 hover:text-gray-900'}`}>Missions</button>
          </div>
          ...
          <div className="flex-grow overflow-y-auto scrollbar-hide p-4 space-y-3">
            {activeTab === 'DRIVERS' ? (
              drivers.map(d => {
                ...
                return (
                  <button key={d.id} onClick={() => focusOnDriver(d)} className={`w-full text-left p-4 rounded-3xl border transition-all ${lockedTargetId === d.id ? 'bg-blue-600 border-blue-400' : 'bg-white border-gray-100 hover:border-blue-200 shadow-sm'}`}>
                    <div className="flex justify-between items-start mb-2">
                      <span className={`font-black text-sm ${lockedTargetId === d.id ? 'text-white' : 'text-gray-900'}`}>{d.name}</span>
                    ...
                    <div className="flex justify-between items-center">
                      <span className={`text-[10px] font-black uppercase tracking-widest ${lockedTargetId === d.id ? 'text-blue-100' : 'text-gray-400'}`}>{d.vehicleNo}</span>
                      <span className={`text-[10px] font-black ${lockedTargetId === d.id ? 'text-white' : 'text-gray-600'}`}>{Math.round(d.lastKnownLocation?.speed || 0)} KM/H</span>
                    </div>
                  </button>
                );
              })
              ...
              activeMissions.length === 0 ? (
                ...
                activeMissions.map(m => (
                  ...
                  }} className="w-full text-left p-5 rounded-[2rem] bg-emerald-50 border border-emerald-100 hover:border-emerald-300 transition shadow-sm">
                    <div className="flex justify-between mb-3">
                       <span className="text-[8px] font-black text-emerald-600 uppercase tracking-[0.2em]">{m.tripType}</span>
                       <span className="text-[8px] font-black text-gray-400 uppercase opacity-50">ID: {m.id}</span>
                    </div>
                    <p className="text-gray-900 font-black text-sm mb-1">{m.origin} → {m.destination}</p>
                    <p className="text-[9px] font-black text-emerald-600/80 uppercase">Assignee: {drivers.find(d => d.id === m.driverId)?.name}</p>
                  </button>
                ))
              )
            )}
      ...
      {selectedJobId && route.length > 0 && (
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-[1000] w-full max-w-3xl px-6">
          <div className="bg-slate-900/95 backdrop-blur-3xl rounded-[3.5rem] p-10 border border-white/10 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)] animate-in slide-in-from-bottom-20">
            ...
            <div className="flex justify-center items-center space-x-10">
              <button onClick={() => setReplayIndex(Math.max(0, replayIndex - 5))} className="w-14 h-14 rounded-2xl bg-white/5 text-gray-400 hover:text-white transition border border-white/5 text-lg"><i className="fas fa-backward-step"></i></button>
              <button onClick={() => setIsReplaying(!isReplaying)} className="w-20 h-20 rounded-3xl bg-white text-slate-950 text-3xl flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition">
                <i className={`fas ${isReplaying ? 'fa-pause' : 'fa-play'}`}></i>
              </button>
              <button onClick={() => setReplayIndex(Math.min(route.length - 1, replayIndex + 5))} className="w-14 h-14 rounded-2xl bg-white/5 text-gray-400 hover:text-white transition border border-white/5 text-lg"><i className="fas fa-forward-step"></i></button>
            </div>
            ...
            <div className="mt-10 flex justify-between text-[10px] font-black text-gray-500 uppercase tracking-widest border-t border-white/5 pt-6">
              ...
              <div className="text-emerald-500">Telemetry Sync v2.5</div>
              <div className="flex items-center space-x-3">
